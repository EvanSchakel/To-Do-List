...
## 2026-06-11 - Handling framework exceptions to prevent DoS and Log Injection
**Vulnerability:** Unhandled framework exceptions like HttpMessageNotReadableException resulted in verbose stack traces being leaked, 500 error flooding logs (potential DoS), and unsanitized exception messages leading to Log Injection vulnerabilities.
**Learning:** Relying purely on a catch-all ExceptionHandler exposes the application to DoS attacks via log flooding on common user errors (e.g., bad JSON input), and directly logging raw exception messages allows attackers to inject CRLF characters.
**Prevention:** Explicitly handle client-side framework exceptions (like HttpMessageNotReadableException, MethodArgumentTypeMismatchException) to return appropriate 4xx statuses, log them as warnings instead of errors to avoid flooding log aggregation tools, and ALWAYS sanitize exception messages (e.g., replacing ,
, 	) before logging them.
