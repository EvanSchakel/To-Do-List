...
## 2024-07-02 - Exception Handler Log Flooding and Information Leakage
**Vulnerability:** The `GlobalExceptionHandler` was catching all exceptions via a generic `Exception.class` handler. Framework-specific client errors (like `HttpMessageNotReadableException` when sending malformed JSON, or `MethodArgumentTypeMismatchException`) were falling through to this generic handler, resulting in a 500 Internal Server Error status and a full stack trace being logged via `logger.error()`.
**Learning:** Returning 500s for client errors is semantically incorrect. More importantly, logging full stack traces for client-induced errors allows an attacker to intentionally spam the server logs by repeatedly sending bad requests (a DoS vector). It also risks leaking internal implementation details if the 500 response body were to ever include the exception message.
**Prevention:** Always implement specific `@ExceptionHandler` methods for common framework client errors. Return the appropriate 400 Bad Request or 404 Not Found status, and use `logger.warn()` to log a concise message without the stack trace.
