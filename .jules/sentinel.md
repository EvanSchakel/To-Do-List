...
## 2026-05-30 - 500 Error Log Flooding via Unhandled Client Exceptions
**Vulnerability:** The application was not explicitly handling framework-specific client errors (such as `HttpMessageNotReadableException`, `MethodArgumentTypeMismatchException`, and `NoResourceFoundException`). Consequently, the generic `Exception` handler caught these, logging them as `logger.error` with full stack traces and returning 500 Internal Server Error.
**Learning:** An attacker can easily trigger these exceptions by sending malformed JSON, invalid data types, or requests to non-existent URLs. This allows them to trivially flood the application logs with stack traces, which degrades performance, wastes log storage, and can mask actual critical production errors.
**Prevention:** Always explicitly handle common framework-specific exceptions in your `@RestControllerAdvice`. Return appropriate 400 (Bad Request) or 404 (Not Found) statuses, and log these client-side errors at the `WARN` or `INFO` level without full stack traces to protect the logging infrastructure.
