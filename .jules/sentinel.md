...
## 2026-06-01 - Exception Handling Information Leakage and Log Flooding
**Vulnerability:** Framework-specific client errors (e.g., `HttpMessageNotReadableException` for bad JSON, `MethodArgumentTypeMismatchException` for invalid types, `NoResourceFoundException` for 404s) were falling through to the generic `Exception` handler. This resulted in 500 Internal Server Errors for what should be 4xx client errors, and flooded the application logs with ERROR-level stack traces. This could lead to a log flooding Denial of Service (DoS) and potential information leakage if error messages are too verbose.
**Learning:** Spring's default behavior, or a overly broad generic exception handler, can turn innocuous client mistakes into high-severity server errors. It's crucial to differentiate between true server faults (500s) and bad client requests (4xxs) at the global exception handling layer.
**Prevention:** Explicitly handle common framework-specific client exceptions in `@RestControllerAdvice`. Return appropriate 400/404 statuses and log the events at the `WARN` level rather than `ERROR`, without dumping full stack traces to the logs.
