...
## 2024-06-12 - Log Injection / Full Stack Trace Exposure via Default Exception Handlers
**Vulnerability:** The application was catching all exceptions via a broad `@ExceptionHandler(Exception.class)` in `GlobalExceptionHandler` and returning a 500 Internal Server Error along with logging the full stack trace. Additionally, client-specific errors (like `HttpMessageNotReadableException` and `NoResourceFoundException`) were falling through to this global handler, logging verbose stack traces and allowing attackers to intentionally flood the application logs (DoS).
**Learning:** Framework-specific client exceptions should be gracefully handled and translated to the appropriate 400/404 HTTP status codes. Additionally, logging raw, unescaped exception messages directly from user input allows CRLF (Carriage Return Line Feed) log injection.
**Prevention:** Explicitly handle common client-side exceptions and map them to their correct HTTP status codes without logging stack traces. Always sanitize external input or exception messages using `.replaceAll("[\r\n\t]", "_")` before logging them.
