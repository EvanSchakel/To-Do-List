...
## 2024-07-07 - Framework Exception Information Leakage and Log Injection
**Vulnerability:** The centralized `GlobalExceptionHandler` was missing explicit handling for framework-specific client errors such as `HttpMessageNotReadableException`, `MethodArgumentTypeMismatchException`, and `NoResourceFoundException`. These were falling back to the default `Exception.class` handler, leaking 500 status codes for client-side issues, creating a DoS vector by flooding error logs with stack traces, and lacking sanitation against CRLF log injection in user-supplied messages.
**Learning:** Returning 500 errors for bad client input is improper error handling. Moreover, logging exception messages (`ex.getMessage()`) without sanitization can expose the application to log injection (CRLF injection) attacks.
**Prevention:** Explicitly map framework exceptions to 400 (Bad Request) or 404 (Not Found) statuses. When logging `ex.getMessage()`, sanitize the input by replacing newlines (`\n`), carriage returns (`\r`), and tabs (`\t`) with safe characters (e.g., underscores) before passing the string to SLF4J.
