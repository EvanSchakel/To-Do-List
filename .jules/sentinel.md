...
## 2024-05-24 - Unhandled Client Errors Causing Log Injection and DoS Risk
**Vulnerability:** The `GlobalExceptionHandler` was catching all exceptions via `Exception.class` and logging them at `ERROR` level, including client-side errors like `HttpMessageNotReadableException`, `MethodArgumentTypeMismatchException`, and `NoResourceFoundException`. Furthermore, `ex.getMessage()` was being logged directly without sanitization, risking CRLF (Log Injection).
**Learning:** Spring Boot routes numerous framework-level validation and routing errors to the generic `Exception` handler if not explicitly handled. This allows attackers to flood the application logs with 500 errors simply by submitting malformed JSON or invalid path parameters, and potentially inject malicious entries via crafted error strings.
**Prevention:** Always handle expected client-driven exceptions (like 400/404) explicitly, log them at `WARN` instead of `ERROR`, and strictly sanitize user-controlled data or exception messages using `.replaceAll("[\\r\\n\\t]", "_")` before passing them to the logger.
