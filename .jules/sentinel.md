...
## 2024-06-14 - Information Leakage and Log Injection via Default Exception Handlers
**Vulnerability:** Unhandled client exceptions (like `HttpMessageNotReadableException`, `MethodArgumentTypeMismatchException`, and `NoResourceFoundException`) defaulted to `500 Internal Server Error` responses, exposing internal stack traces in logs. Furthermore, exception messages were logged directly, creating a CRLF Log Injection vulnerability if the user input contained newlines.
**Learning:** Default global exception handlers must explicitly catch and handle framework-level client errors. Failing to do so causes "Error" level logs with massive stack traces for benign client mistakes (a DoS vector) and risks Log Injection.
**Prevention:** Explicitly map client-side exceptions to 400/404 responses. Additionally, always sanitize user-provided or exception message strings by replacing `\r`, `\n`, and `\t` with underscores before passing them to the logger.
