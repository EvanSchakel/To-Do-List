...
## 2026-06-06 - Prevent Error Log Flooding and 500 Status on Client Errors
**Vulnerability:** The `GlobalExceptionHandler` was catching all unhandled exceptions, including framework-specific client errors like `HttpMessageNotReadableException`, `MethodArgumentTypeMismatchException`, and `NoResourceFoundException`, with a generic `Exception.class` handler. This resulted in returning a 500 Internal Server Error for client-side mistakes (e.g., malformed JSON) and logging full stack traces, which could lead to log flooding and potential Denial of Service (DoS).
**Learning:** Framework-specific client errors need explicit handling to map them to appropriate 4xx HTTP statuses (e.g., 400 Bad Request, 404 Not Found) instead of 500 Internal Server Error.
**Prevention:** Always add specific `@ExceptionHandler` methods for common client-side exceptions (like `HttpMessageNotReadableException`, `MethodArgumentTypeMismatchException`, `NoResourceFoundException`) in global exception handlers. Return appropriate 400/404 statuses and log only warnings with the error message to prevent internal stack trace leakage and 500 error log flooding.
