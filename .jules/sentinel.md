...
## 2024-07-03 - Exception Handling and Log Injection Vulnerability
**Vulnerability:** Unhandled framework client exceptions like `HttpMessageNotReadableException`, `MethodArgumentTypeMismatchException`, and `NoResourceFoundException` would trigger a 500 Internal Server Error, potentially leaking stack traces and flooding the application error logs (DoS). Additionally, printing the unsanitized `ex.getMessage()` when logging these errors created a Log Injection (CRLF Injection) vulnerability.
**Learning:** Framework-specific client errors should be caught globally and transformed into structured 400 Bad Request or 404 Not Found JSON responses instead of causing unexpected 500 errors. Also, unvalidated user input included in exception messages can inject newlines and spoof log entries if not properly sanitized.
**Prevention:** Explicitly handle common client exceptions in `@RestControllerAdvice`, map them to the correct HTTP status, and always sanitize `ex.getMessage()` by replacing carriage returns (`\r`), newlines (`\n`), and tabs (`\t`) with underscores before logging them.
