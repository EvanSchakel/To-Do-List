...
## 2026-06-21 - Exception Handling Information Leakage
**Vulnerability:** The application was leaking potentially sensitive framework details or generating log spam by returning 500 Internal Server Error with generic stack traces (or unhandled exceptions) for client-side errors like `HttpMessageNotReadableException`, `MethodArgumentTypeMismatchException`, and `NoResourceFoundException`. It also didn't explicitly sanitize unexpected exception messages before logging them.
**Learning:** Spring's default exception handling, or a too-simple `@ExceptionHandler(Exception.class)`, can treat user-fault errors (bad JSON formats, wrong parameter types, 404s) as 500 Internal Server Errors, flooding the logs and masking real issues, while potentially allowing log injection vulnerabilities via un-sanitized log messages.
**Prevention:** Always implement a `GlobalExceptionHandler` that explicitly catches client-side framework exceptions (like `HttpMessageNotReadableException`), returning appropriate 400/404 statuses with safe, generic messages. Ensure all logged user inputs or exception messages are sanitized to prevent CRLF log injection.
