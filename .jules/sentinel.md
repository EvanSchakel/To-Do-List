...
## 2024-06-26 - Log Flooding and CRLF Log Injection
**Vulnerability:** The `GlobalExceptionHandler.java` logged unhandled exceptions, which meant client-side errors like `HttpMessageNotReadableException`, `MethodArgumentTypeMismatchException`, or `NoResourceFoundException` resulted in full stack traces and 500 statuses. This allowed an attacker to flood the logs with excessive errors (Denial of Service) and potentially inject fake log entries by sending newline characters `\r\n` within malicious payloads.
**Learning:** Default exception handlers will treat unhandled client errors as server errors, flooding logs and potentially leaking internal stack traces. Unsanitized user inputs recorded in logs are vulnerable to CRLF injection.
**Prevention:** Explicitly handle framework-specific client errors (e.g., `HttpMessageNotReadableException`) to return 400/404 statuses and log only warnings. Sanitize exception messages by replacing carriage returns (`\r`), newlines (`\n`), and tabs (`\t`) with safe characters (like underscores) before logging them via SLF4J to prevent log injection.
