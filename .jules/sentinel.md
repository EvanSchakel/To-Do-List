...
## 2026-06-25 - Prevent Log Injection and Error Flooding
**Vulnerability:** Framework-specific client errors (`HttpMessageNotReadableException`, `MethodArgumentTypeMismatchException`, `NoResourceFoundException`) were bubbling up to the general `Exception.class` handler. This exposed internal details, caused 500 status codes for bad requests, and lacked log sanitization, creating risks for log injection (CRLF injection) and error log flooding (DoS).
**Learning:** Spring Boot's global exception handling must explicitly catch specific framework exceptions that represent client-side mistakes (4xx errors) rather than treating everything as an unhandled 500 error.
**Prevention:** Explicitly handle these exceptions to return 400/404 statuses. When logging these errors as warnings, explicitly sanitize exception messages (e.g., `ex.getMessage().replaceAll("[\r\n\t]", "_")`) to prevent log injection vulnerabilities.
