...
## 2024-05-29 - Item-specific Caching and Optional<T> SpEL Evaluation
**Learning:** When adding `@Cacheable` to methods returning `Optional<T>` (e.g., `getTaskById`), attempting to prevent caching of empty results using SpEL expressions like `unless = "#result.isEmpty()"` or `!#result.isPresent()` causes a `SpelEvaluationException`. This occurs because Spring 4.3+ automatically unwraps the `Optional` to evaluate and cache the underlying value, so `#result` refers to the actual entity (which does not have an `isEmpty()` method) rather than the `Optional` wrapper.
**Action:** When implementing item-specific caching for `Optional<T>` endpoints alongside collection caches (requiring multiple `@CacheEvict` via `@Caching`), avoid `Optional`-specific SpEL methods in `unless`. Omit the `unless` clause, or safely use `#result == null`.
