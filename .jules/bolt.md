...
## 2026-06-20 - Item-Specific Caching Alongside Collection Caching
**Learning:** When a list endpoint (`getAllTasks()`) is cached globally (e.g., `tasks`), adding item-specific caching (e.g., `getTaskById()`) requires careful invalidation. If mutation methods (update, delete) only evict the global list cache, the item-specific cache becomes stale, leading to inconsistent data when retrieving an item directly by ID after an update.
**Action:** When implementing item-specific caching alongside collection caching, ensure mutation methods (update, delete) use `@Caching(evict = { ... })` with multiple `@CacheEvict` annotations to simultaneously clear both the specific item (`@CacheEvict(value = "task", key = "#id")`) and the global list cache (`@CacheEvict(value = "tasks", allEntries = true)`).
