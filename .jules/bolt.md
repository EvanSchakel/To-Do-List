...
## 2026-06-18 - Caching Specific Items requires Simultaneous Multi-Eviction
**Learning:** If you add caching to an individual item lookup like `getTaskById(Long id)` while maintaining a global collection cache for operations like `getAllTasks()`, any update or delete actions must invalidate BOTH the specific item cache and the global collection cache. Failing to do so causes data inconsistency where lists are updated, but fetching a specific ID returns stale data.
**Action:** When adding `@Cacheable(value = "task", key = "#id")`, explicitly update mutating methods to use `@Caching` housing multiple `@CacheEvict` directives (e.g., `@Caching(evict = { @CacheEvict(value = "tasks", allEntries = true), @CacheEvict(value = "task", key = "#id") })`) to flush both cache pools. Avoid using SpEL conditions like `#result == null` or `#result.isPresent()` on `Optional` return types as Spring unboxes the `Optional` first, causing `SpelEvaluationException`.
