...
## 2026-06-25 - Item-Specific Caching in Spring Data Services
**Learning:** When a global list cache (like `getAllTasks()`) is already implemented, individual item lookups (`getTaskById()`) might still bypass the cache and hit the database redundantly if they aren't explicitly cached. However, when caching individual items, it's critical that the eviction strategy on mutation methods (update, delete) is updated to clear *both* the global list cache and the specific item cache, using `@Caching(evict = { ... })`, otherwise stale item data will be served.
**Action:** When adding `@Cacheable(value = "item", key = "#id")` to read methods, immediately audit all write/delete methods and wrap existing `@CacheEvict` annotations inside a `@Caching` block to evict both `"items"` and `"item"`.
