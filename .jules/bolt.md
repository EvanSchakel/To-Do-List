...
## 2026-07-04 - Item-Specific Caching with Spring Cache
**Learning:** While collection caching (e.g., caching `getAllTasks()`) reduces database load for list endpoints, frequently fetching individual items via `getTaskById()` still hits the database unnecessarily. Furthermore, when implementing item-specific caching alongside list caching, mutation methods (update, delete) must explicitly evict *both* caches to prevent stale data. Spring's `@Caching` annotation allows grouping multiple `@CacheEvict` statements.
**Action:** Annotate individual lookup methods with `@Cacheable(value = "item_cache", key = "#id")`. Update corresponding mutation methods using `@Caching(evict = { @CacheEvict(value = "list_cache", allEntries = true), @CacheEvict(value = "item_cache", key = "#id") })` to ensure complete cache consistency.
