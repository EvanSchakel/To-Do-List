...
## 2026-05-29 - Item-Level Cache Eviction
**Learning:** When implementing item-specific caching (e.g., caching individual lookups with `@Cacheable(value = "task", key = "#id")`) alongside collection caching, mutation methods (update, delete) must simultaneously clear both the specific item and the global list cache to avoid stale data.
**Action:** Ensure mutation methods (update, delete) use `@Caching(evict = { ... })` with multiple `@CacheEvict` annotations to clear both caches. Entity creation only needs to evict the global collection cache.
