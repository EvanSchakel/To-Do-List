...
## 2026-07-02 - Item-Specific Caching in Spring Data Services
**Learning:** When a list endpoint is cached (e.g., `getAllTasks()`), adding item-level caching (`@Cacheable(value = "task", key = "#id")` to `getTaskById()`) reduces database queries for frequently accessed individual items. However, to maintain cache coherence, all write methods (update, delete) must explicitly evict *both* the item cache and the global collection cache.
**Action:** Use `@Caching(evict = { @CacheEvict(value = "tasks", allEntries = true), @CacheEvict(value = "task", key = "#id") })` on mutation methods to ensure both collection-level and item-level caches are invalidated simultaneously.
